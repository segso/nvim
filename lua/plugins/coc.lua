return {
    ...
    config = function()
        ...
        keymap("i", "<TAB>", 'coc#pum#visible() ? coc#pum#confirm() : coc#expandableOrJumpable() ? "<C-r>=coc#rpc#request(\'doKeymap\', [\'snippets-expand-jump\',\'\'])<CR>" : v:lua.check_back_space() ? "<TAB>" : coc#refresh()', opts);
        ...
        keymap("n", "[d", "<Plug>(coc-diagnostic-prev)", {silent = true})
        keymap("n", "]d", "<Plug>(coc-diagnostic-next)", {silent = true})
        ...
        -- keymap("n", "<leader>qf", "<Plug>(coc-diagnostic-next)<Plug>(coc-fix-current)", opts)
        keymap("n", "<leader>qf", "<Plug>(coc-diagnostic-next) | <Plug>(coc-fix-current)", opts)
        ...
        -- Show commands
        keymap("n", "<space>g", ":<C-u>CocList commands<cr>", opts)
